...
const tcpcontroller_1 = require("./tcp/tcpcontroller");
const tcpController = new tcpcontroller_1.TCPController(63000, '0.0.0.0');
...
const sendUpdate = (id, x, y) => {
    console.log(`${id} (${x}, ${y})`);
    let messageBuffer = Buffer.alloc(16, '\0', 'hex');
...
bots.forEach((bot) => {
    setInterval(() => {
        ...
        if (bot.x > bounds.xmax)
            bot.x = 0;
        if (bot.y > bounds.ymax)
            bot.y = 0;
        if (bot.x < 0)
            bot.x = bounds.xmax;
        if (bot.y < 0)
